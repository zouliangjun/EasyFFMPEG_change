...
void FEasyFFMPEGModule::InitLibraryHandles()
{
	...
	if (AVCodecHandle == nullptr || //AVDeviceHandle == nullptr ||
...
void* FEasyFFMPEGModule::LoadDependencyLibrary(const FString& DLLName)
{
	...
	FString dllDir = FPaths::Combine(*baseDir, TEXT("ThirdParty/ffmpeg/bin/vs/x64"));
...
void* FEasyFFMPEGModule::LoadDependencySOLibrary(const FString& SOName)
{
	//FString baseDir = IPluginManager::Get().FindPlugin("FFMPEGMedia")->GetBaseDir();

	//FString soDir = FPaths::Combine(*baseDir, TEXT("ThirdParty/ffmpeg/libs/android/arm64-v8a"));

	//FString soFilename = FPaths::Combine(*soDir, *SOName);

	//UE_LOG(LogFFmpeg, Log, TEXT("Loading dependency SO ----> %s"), *soFilename);


	//return FPlatformProcess::GetDllHandle(*soFilename);
	return nullptr;
}
